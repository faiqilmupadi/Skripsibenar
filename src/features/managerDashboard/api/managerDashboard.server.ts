...
export async function itemFsnHandler(_req?: Request) {
  try {
    const marks = OUTBOUND_TYPES.map(() => "?").join(",");
    const sql = `SELECT mm.materialDescription name,ms.freeStock,
      ...
      GROUP BY mm.partNumber,ms.plant ORDER BY keluar DESC LIMIT 10`;
    const list = await q<any[]>(sql, OUTBOUND_TYPES);
...
export async function assetTrendHandler(_req?: Request) {
  try {
    const sql = `SELECT d.postingDate date,
